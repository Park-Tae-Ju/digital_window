...
배경 영상을 읽는 작업을 별도의 스레드(`BackgroundVideoReader`)로 분리하여, 메인 루프가 영상 디코딩을 기다리지 않고 항상 최신 프레임을 즉시 가져올 수 있도록 한 버전입니다.  
이전 버전들은 메인 루프에서 직접 `cap.read()`를 호출했기 때문에, 디코딩 지연이 발생하면 화면 반응성도 함께 떨어지는 문제가 있었습니다.
...
실험 원본 두 파일은 영상 및 창틀 프레임 등의 경로만 제외하고, `experiments/` 폴더에 남겨봤습니다.  
